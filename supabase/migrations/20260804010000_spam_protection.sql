...
-- 2. Update the RPC to include rate limiting
...
BEGIN
    ...
    -- Safely extract x-forwarded-for header
    BEGIN
        v_ip := current_setting('request.headers', true)::json->>'x-forwarded-for';
        IF v_ip IS NOT NULL THEN
            v_ip := split_part(v_ip, ',', 1); -- Get the first IP in case of a list
            v_ip := trim(v_ip);
        END IF;
    ...
    IF v_ip IS NOT NULL AND v_ip != 'unknown' THEN
        ...
        IF v_attempt_count IS NOT NULL THEN
            -- Reset limit if more than 24 hours have passed
            ...
            -- Block if 3 or more attempts within 24 hours
